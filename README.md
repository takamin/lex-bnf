...
Here is a sample for the sqlish query feature of
the npm [aws-node-util](https://www.npmjs.com/package/aws-node-util):
...
function parseSqlishQuery(source) {
    ...
    let tokens = BNF.tokenize(source, lexToken);
    ...
    let bnf = new BNF("sqlish-query", BNF);
